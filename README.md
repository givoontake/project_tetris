...
측정값은 **클라이언트의 입력 송신부터 해당 응답 수신까지의 지연 시간**입니다. 로그인과 경기 결과 저장은 우회하고, 지속적인 입력 처리를 위한 테스트 모드를 사용했습니다. 실제 경기 전체 및 데이터베이스를 포함한 서비스 수용량과는 구분합니다.
...
이 측정에서는 로그인 처리를 우회하고 경기 결과 저장은 수행했습니다. 게임 틱은 목표 빈도를 달성했지만, 데이터베이스 작업의 평균 큐 대기 시간이 약 41초로 누적되었습니다. 게임 처리 성능과 저장 작업의 처리 용량을 별도로 확인한 결과입니다.
